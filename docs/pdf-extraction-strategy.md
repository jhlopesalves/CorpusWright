...
CorpusWright uses PDFium, which contains a complete, battle-tested PDF rendering engine. PDFium automatically processes embedded font CMaps, resolves glyph encoding, and returns fully decoded Unicode character strings.
...
CorpusWright queries `text_page.chars()`, which extracts characters from all text contexts on the page, including text nested within Form XObjects.
...
### 4. Midpoint Two-Column Scrambling
...
CorpusWright defaults to `PdfiumFlat` (the standard stream order) or `PdfiumVisualSingleColumn` (which groups lines by Y coordinates and sorts left-to-right). The two-column parser (`PdfiumVisualColumnsExperimental`) is kept strictly internal, experimental, and warning-heavy.
...
An experimental strategy for two-column page layouts. Currently unsupported; falls back to visual single-column extraction to prevent reading order regression.
...
To clean up artefacts specific to PDF documents (such as running headers, footers, and page numbers/labels) without modifying the original source files, CorpusWright provides optional, deterministic, page-aware PDF post-extraction cleanup options.
